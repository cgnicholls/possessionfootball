...
def create_network(input_dim, num_hidden, num_players):    
    ...
    num_input = tf.shape(input_layer)[0]
    all_positions = input_layer[:,2:]
    defender_positions = input_layer[:,:2]
    tf.reshape(all_positions, 
    keep_prob = tf.placeholder(tf.float32, name='keep_prob')
    with tf.name_scope('layer1'):
        hidden1, l2_hidden1 = nn_layer(small_hidden_layer, input_dim, num_hidden, 'layer1')
...
def qlearning():
    ...
    tf_output_layer, l2_reg = create_network((NUM_PLAYERS)*2, 20, NUM_ACTIONS)
